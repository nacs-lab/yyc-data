...
const iname_a = joinpath(@__DIR__, "data", "data_20170919_184604.csv")
...
idxs, ratios, uncs = group_time(params, counts, (1:7, 8:14, 15:21, 22:28), 812 * 3)

const prefix = joinpath(@__DIR__, "imgs", "data_20170919_184604_time")

figure()
errorbar(idxs, ratios[:, 2], uncs[:, 2], fmt=".-", label="X")
errorbar(idxs, ratios[:, 4], uncs[:, 4], fmt=".-", label="Y")
grid()
ylim([0, 0.22])
legend()
xlabel("Initial sequence index")
ylabel("Total cooling sideband")
NaCsPlot.maybe_save("$(prefix)_cool")

figure()
errorbar(idxs, ratios[:, 1], uncs[:, 1], fmt=".-", label="X")
errorbar(idxs, ratios[:, 3], uncs[:, 3], fmt=".-", label="Y")
grid()
ylim([0, 1])
legend()
xlabel("Initial sequence index")
ylabel("Total heating sideband")
NaCsPlot.maybe_save("$(prefix)_heat")
