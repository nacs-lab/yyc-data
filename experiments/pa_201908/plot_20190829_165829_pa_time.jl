...
function load_data(names, selector)
    ...
    for name in names
        param, counts = matopen(joinpath(@__DIR__, "data", "data_$name.mat")) do fd
            ...
            ts = sg["base"]["vars"]["params"]["Merge"]["PA"]["T"]
...
const names_files = ["names_20190829_165829.mat"]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20190829_165829_pa_time")

fit_50 = fit_survival(model_lorentzian, datas[1][:, 2], [0.7, 0.35, 496.1, 0.2])
...
text(495.91, 0.72, "\$\\Gamma_{PA}=$(fit_50.uncs[4] * 1000)\$ MHz", color="C1")
text(495.91, 0.83, "\$f_{PA}=$(fit_50.uncs[3])\$ GHz", color="C1")
grid()
ylim([0.3, 0.95])
xlim([495.9, 496.34])
