...
const names_files = ["names_20190828_211824.mat"]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20190828_211824_pa_pwr")

fit_pa = fit_survival(model_lorentzian, datas[1][:, 2], [0.7, 0.35, 496.1, 0.2])

figure()
NaCsPlot.plot_survival_data(datas[1][:, 1], fmt="C0.-", label="No PA")
NaCsPlot.plot_survival_data(datas[1][:, 2], fmt="C1.", label="PA")
plot(fit_pa.plotx, fit_pa.ploty, "C1-")
text(496.25, 0.32, "\$\\Gamma_{PA}=$(fit_pa.uncs[4] * 1000)\$ MHz", color="C1")
text(496.25, 0.43, "\$f_{PA}=$(fit_pa.uncs[3])\$ GHz", color="C1")
grid()
ylim([0.3, 0.95])
...
title("PA spectrum (two-body no PA)")
xlabel("306XXX GHz")
ylabel("Two-body survival")
NaCsPlot.maybe_save("$(prefix)_2b_nopa")
...
xlabel("306XXX GHz")
ylabel("Two-body survival")
NaCsPlot.maybe_save("$(prefix)_2b_pa")

figure()
NaCsPlot.plot_survival_data(datas_cscs[1][:, 1], fmt="C2.-", label="Cs No PA")
NaCsPlot.plot_survival_data(datas_nana[1][:, 1], fmt="C1.-", label="Na No PA")
NaCsPlot.plot_survival_data(datas_cscs[1][:, 2], fmt="C2.--", label="Cs PA")
NaCsPlot.plot_survival_data(datas_nana[1][:, 2], fmt="C1.--", label="Na PA")
grid()
ylim([0.3, 0.95])
legend(fontsize="small")
