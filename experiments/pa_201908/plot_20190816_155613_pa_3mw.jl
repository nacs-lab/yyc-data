...
const names_files = ["names_20190816_155613.mat"]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20190816_155613_pa_3mw")

for ds in [datas_cs, datas_na, datas, datas_cscs, datas_nana]
    ds[1] = NaCsData.map_params((i, v)->v - 3000, ds[1])
end

fit = fit_survival(model_lorentzian, datas[1], [0.8, 0.3, 255.45, 0.1])
...
text(255.29, 0.38, "\$\\Gamma=$(fit.uncs[4] * 1000)\$ MHz", color="C0")
grid()
ylim([0.35, 0.95])
