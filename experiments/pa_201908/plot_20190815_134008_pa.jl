...
const names_files = ["names_20190815_134008.mat"]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20190815_134008_pa")

fit_20 = fit_survival(model_lorentzian, datas[1][:, 1], [0.8, 0.3, 255.45, 0.4])
fit_50 = fit_survival(model_lorentzian, datas[1][:, 2], [0.8, 0.3, 255.45, 0.4])
...
text(254.6, 0.81, "\$\\Gamma_{20}=$(fit_20.uncs[4])\$ GHz", color="C0")
text(254.6, 0.51, "\$\\Gamma_{50}=$(fit_50.uncs[4] * 1000)\$ MHz", color="C1")
grid()
ylim([0.35, 0.95])
...
title("PA spectrum (two-body 20 ms)")
xlabel("309XXX GHz")
ylabel("Two-body survival")
NaCsPlot.maybe_save("$(prefix)_2b_20")
...
ylim([0.35, 0.95])
legend(fontsize="small")
title("PA spectrum (two-body 50 ms)")
xlabel("309XXX GHz")
ylabel("Two-body survival")
NaCsPlot.maybe_save("$(prefix)_2b_50")
