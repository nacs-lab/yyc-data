...
const inames = ["data_20200524_161707.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs = [([0; [0.04, 0.07, 0.10, 0.135, 0.17, 0.22, 0.26] * 2.5 .- 0.01], # 6 mW, 770.487 MHz
                [0.0], # 6 mW, 0 ms
                487 .+ (-2.5:0.5:2.5), # 6 mW, 0.23 ms
                487 .+ (-2.5:0.5:2.5), # 6 mW, 0.42 ms
                ),
               ]
...
const data_fit = [NaCsData.map_params((i, v) -> (v, 0.0, 1), data_nacs_00);
                  ...
                  NaCsData.map_params((i, v) -> (487, v, 1), data_nacs_t)]

const prefix = joinpath(@__DIR__, "imgs", "fit_20200524_161707_raman_3322")
...
fit = fit_survival(model, data_fit, [487, 2π * 1.5, 0, 2π / 0.2, 0.1, 0.3],
                   plotx=false, lower=[0.0, 0.0, 0.0, 0, 0, 0])
@show fit.uncs
const param_1 = get_model_param(fit.param, 1)
const uncs_1 = get_model_param(fit.uncs, 1)

const plot_freq_lo = 487 - 3
const plot_freq_hi = 487 + 3
...
NaCsPlot.plot_survival_data(data_nacs_t, fmt="C0.", label="770.487 MHz")
plot(plot_time, model_2d.(plot_time, 487, (param_1,)), "C0")
title("Time")
legend(fontsize="x-small", loc="upper right")
text(0.12, 0.12, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
