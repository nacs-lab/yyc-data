...
const inames = ["data_20200501_203952.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs = [(505.00 .+ [-30; -3.5:0.7:3.5; 30], # 15 mW, 0.21 ms
                447.20 .+ [-20; -2.5:0.5:2.5; 20], # 12 mW, 0.26 ms
                391.00 .+ [-10; -1.5:0.3:1.5; 10], #  9 mW, 0.66 ms
                332.30 .+ [-8; -1.25:0.25:1.25; 8], # 6 mW, 1.01 ms
                268.50 .+ [-5; -0.75:0.15:0.75; 5], # 3 mW, 2.51 ms
                )]
...
const data_nacs_15 = datas_nacs[1][1]
const data_nacs_12 = datas_nacs[1][2]
const data_nacs_9 = datas_nacs[1][3]
const data_nacs_6 = datas_nacs[1][4]
const data_nacs_3 = datas_nacs[1][5]

const prefix = joinpath(@__DIR__, "imgs", "data_20200501_203952_raman_det_3322")
...
fit15 = fit_survival(model_lorentzian, data_nacs_15, [0.4, 0.3, 505.0, 8])
fit12 = fit_survival(model_lorentzian, data_nacs_12, [0.4, 0.3, 447.2, 5])
fit9 = fit_survival(model_lorentzian, data_nacs_9, [0.4, 0.3, 391, 3])
fit6 = fit_survival(model_lorentzian, data_nacs_6, [0.3, 0.25, 332.3, 2.5])
fit3 = fit_survival(model_lorentzian, data_nacs_3, [0.4, 0.3, 268.5, 2])
...
text(477, 0.332, "\$f=$(770 + fit15.uncs[3] / 1000)\\ MHz\$")
title("288510 GHz, 15 mW, 0.20 ms")
grid()
...
text(428, 0.26, "\$f=$(770 + fit12.uncs[3] / 1000)\\ MHz\$")
title("288510 GHz, 12 mW, 0.25 ms")
grid()
xlabel("2-Photon Detuning (770XXX kHz)")
ylabel("Two-body survival")
...
text(381.5, 0.24, "\$f=$(770 + fit9.uncs[3] / 1000)\\ MHz\$")
title("288510 GHz, 9 mW, 0.65 ms")
grid()
xlabel("2-Photon Detuning (770XXX kHz)")
ylabel("Two-body survival")
...
text(324, 0.30, "\$f=$(770 + fit6.uncs[3] / 1000)\\ MHz\$")
title("288510 GHz, 6 mW, 1.0 ms")
grid()
xlabel("2-Photon Detuning (770XXX kHz)")
ylabel("Two-body survival")
...
text(263, 0.28, "\$f=$(770 + fit3.uncs[3] / 1000)\\ MHz\$")
title("288510 GHz, 3 mW, 2.5 ms")
grid()
xlabel("2-Photon Detuning (770XXX kHz)")
ylabel("Two-body survival")
...
grid()
title("Raman Resonance (288510 GHz)")
