...
const inames = ["data_20200523_183654.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs = [([0; [0.04, 0.07, 0.10, 0.135, 0.17, 0.22,
                     0.26] .* 0.8 .- 0.01], # 15 mW, 770.886 MHz
                [0.0], # 15 mW, 0 ms
                886 .+ (-15:3.0:15), # 15 mW, 0.135 ms
                886 .+ (-15:3.0:15), # 15 mW, 0.076 ms
                ),
               ]
...
const data_nacs_066 = datas_nacs[1][4] # Survival 1

const data_fit = [NaCsData.map_params((i, v) -> (886.0, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (v, 0.125, 1), data_nacs_125);
                  NaCsData.map_params((i, v) -> (v, 0.066, 1), data_nacs_066);
                  NaCsData.map_params((i, v) -> (886.0, v, 1), data_nacs_t)]

const prefix = joinpath(@__DIR__, "imgs", "fit_20200523_183654_raman_3322")
...
fit = fit_survival(model, data_fit, [886.0, 2π * 1.5, 0, 2π / 0.2, 0.1, 0.3],
                   plotx=false, lower=zeros(6))
@show fit.uncs
const param_1 = get_model_param(fit.param, 1)
const uncs_1 = get_model_param(fit.uncs, 1)

const plot_freq_lo = 886.0 - 16
const plot_freq_hi = 886.0 + 16
...
NaCsPlot.plot_survival_data(data_nacs_066, fmt="C1.", label="0.066 ms")
plot(plot_freq, model_2d.(0.066, plot_freq, (param_1,)), "C1")
...
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.886 MHz")
plot(plot_time, model_2d.(plot_time, 886.0, (param_1,)), "C0")
title("Time")
