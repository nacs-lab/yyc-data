...
const inames = ["data_20200704_224625.mat",
                "data_20200705_103505.mat",
...
const specs = [([0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16, 0.175, 0.19, 0.22, 0.25,
                 0.265, 0.28] .* 2.5 .- 0.01, # 6 mW, 770.098 MHz
                [0.0], # 6 mW, 0 ms
                98.0 .+ (-3:0.6:3), # 6 mW, 0.22 ms
                98.0 .+ (-5:1:5), # 6 mW, 0.43 ms
                ),
               ([0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16, 0.175, 0.19, 0.22, 0.25,
                 0.265, 0.28] .* 2.5 .- 0.01, # 6 mW, 770.098 MHz
                [0.0], # 6 mW, 0 ms
                98.0 .+ (-3:0.6:3), # 6 mW, 0.22 ms
                98.0 .+ (-5:1:5), # 6 mW, 0.43 ms
...
const data_nacs_21 = [datas_nacs[1][3]; datas_nacs[2][3]] # Survival 1
const data_nacs_42 = [datas_nacs[1][4]; datas_nacs[2][4]] # Survival 1
...
const data_fit = [NaCsData.map_params((i, v) -> (1, 98.0, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 0.21, 1), data_nacs_21);
                  NaCsData.map_params((i, v) -> (1, v, 0.42, 1), data_nacs_42);
                  NaCsData.map_params((i, v) -> (1, 98.0, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200704_224625_raman6_3322_2")
...
fit = fit_survival(model, data_fit, [98.0, 2π * 1.5, 0, 2π / 0.2, 0, 0, 0.1,
                                     0.3, 0.8, 0.3, 0.3, 0.3],
...
const plot_freq_lo = 98.0 - 6
const plot_freq_hi = 98.0 + 6
...
title("288895 GHz, 6 mW")

subplot(2, 2, 2)
const plot_time = linspace(0, 0.70, 1000)
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.098 MHz")
plot(plot_time, model_2d.(plot_time, 98.0, (param_1,)), "C0")
title("Time")
