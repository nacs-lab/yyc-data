...
const inames = ["data_20200721_203401.mat",
                "data_20200722_101121.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int)]
const specs = [([0.02, 0.04, 0.07, 0.085, 0.10, 0.13, 0.145, 0.16, 0.175, 0.19, 0.22,
                 0.25] .* 3 .- 0.01, # 15 mW, 770.4895 MHz
                [0.0], # 15 mW, 0 ms
                489.5 .+ [-10; -2.5:0.5:2.5; 10], # 15 mW, 0.21 ms
                489.5 .+ [-10; -1.5:0.3:1.5; 10], # 15 mW, 0.43 ms
                ),
               ([0.0, 2, 4, 10],
                [0.0, 25, 50, 100]),
               ]
...
const data_pa_na = [datas_na[2][2]; datas_na[2][1]]
const data_pa_cs = datas_cs[2][2]
const data_pa_a = datas_nacs[2][2]
const data_pa_m = datas_nacs[2][1]

const data_fit = [NaCsData.map_params((i, v) -> (1, 489.5, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 0.20, 1), data_nacs_20);
                  NaCsData.map_params((i, v) -> (1, v, 0.42, 1), data_nacs_42);
                  NaCsData.map_params((i, v) -> (1, 489.5, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200721_203401_raman15_3322_2")
...
fit = fit_survival(model, data_fit, [489.5, 2π * 1.5, 0, 2π / 0.2, 0, 0, 0.1,
                                     0.3, 0.8, 0.3, 0.3, 0.3],
...
const plot_freq_lo = 489.5 - 16
const plot_freq_hi = 489.5 + 16
...
subplot(2, 2, 2)
const plot_time = linspace(0, 0.25 * 4, 1000)
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.4895 MHz")
plot(plot_time, model_2d.(plot_time, 489.5, (param_1,)), "C0")
title("Time")
text(0.17, 0.15, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
