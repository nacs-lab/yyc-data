...
const inames = ["data_20200706_141802.mat",
                "data_20200706_002029.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int)]
const specs = [([0.04, 0.07, 0.10, 0.13, 0.16, 0.19, 0.22, 0.25,
                 0.28] * 6 .- 0.01, # 3 mW, 770.1517 MHz
                [0.0], # 3 mW, 0 ms
                151.7 .+ (-1.5:0.3:1.5), # 3 mW, 0.71 ms
                151.7 .+ (-2:0.4:2), # 3 mW, 1.31 ms
                ),
               ([0.0, 2, 4, 10],
                [0.0, 20, 40, 80, 120],
                [0.0, 20, 40, 80, 160] .* 2,
...
const data_nacs_70 = datas_nacs[1][3] # Survival 1
const data_nacs_130 = datas_nacs[1][4] # Survival 1
...
const data_fit = [NaCsData.map_params((i, v) -> (1, 151.7, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 0.70, 1), data_nacs_70);
                  NaCsData.map_params((i, v) -> (1, v, 1.3, 1), data_nacs_130);
                  NaCsData.map_params((i, v) -> (1, 151.7, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200706_141802_raman3_3322_2")
...
fit = fit_survival(model, data_fit, [151.7, 2π * 1.0, 0, 2π * 0.3, 0, 0, 0.2,
...
const plot_freq_lo = 151.7 - 2.1
const plot_freq_hi = 151.7 + 2.1
...
NaCsPlot.plot_survival_data(data_nacs_70, fmt="C1.", label="0.7 ms")
plot(plot_freq, model_2d.(0.70, plot_freq, (param_1,)), "C1")
NaCsPlot.plot_survival_data(data_nacs_130, fmt="C2.", label="1.3 ms")
plot(plot_freq, model_2d.(1.3, plot_freq, (param_1,)), "C2")
legend(fontsize="x-small", loc="lower left")
xlim([plot_freq_lo, plot_freq_hi])
grid()
xlabel("2-Photon Detuning (770XXX kHz)")
ylabel("Two-body survival")
title("288895 GHz, 3 mW")

subplot(2, 2, 2)
const plot_time = linspace(0, 1.70, 1000)
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.1517 MHz")
plot(plot_time, model_2d.(plot_time, 151.7, (param_1,)), "C0")
title("Time")
text(0.40, 0.16, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
