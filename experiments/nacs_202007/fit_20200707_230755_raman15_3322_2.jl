...
const inames = ["data_20200707_230755.mat",
                "data_20200708_211323.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int)]
const specs = [([0.04, 0.07, 0.085, 0.10, 0.13, 0.145, 0.16, 0.175, 0.19, 0.22,
                 0.25] .- 0.01, # 15 mW, 769.897 MHz
                [0.0], # 15 mW, 0 ms
                -103.0 .+ (-15:3.0:15), # 15 mW, 0.17 ms
                -103.0 .+ (-10:2.0:10), # 15 mW, 0.09 ms
                ),
               ([0.0, 2, 4, 10],
                [0.0, 20, 40, 80, 120],
                [0.0, 20, 40, 80, 160] .* 2,
...
const data_nacs_16 = datas_nacs[1][3] # Survival 1
...
const data_fit = [NaCsData.map_params((i, v) -> (1, -103.0, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 0.16, 1), data_nacs_16);
                  NaCsData.map_params((i, v) -> (1, v, 0.08, 1), data_nacs_08);
                  NaCsData.map_params((i, v) -> (1, -103.0, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200707_230755_raman15_3322_2")
...
fit = fit_survival(model, data_fit, [-103.0, 2π * 1.5, 0, 2π / 0.2, 0, 0, 0.1,
                                     0.3, 0.8, 0.3, 0.3, 0.3],
...
const plot_freq_lo = -103.0 - 16
const plot_freq_hi = -103.0 + 16
...
NaCsPlot.plot_survival_data(data_nacs_16, fmt="C2.", label="0.16 ms")
plot(plot_freq, model_2d.(0.16, plot_freq, (param_1,)), "C2")
legend(fontsize="x-small", loc="lower right")
ylim([0, ylim()[2]])
grid()
xlabel("2-Photon Detuning (770XXX kHz)")
ylabel("Two-body survival")
title("288888 GHz, 15 mW")

subplot(2, 2, 2)
const plot_time = linspace(0, 0.25, 1000)
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="769.897 MHz")
plot(plot_time, model_2d.(plot_time, -103.0, (param_1,)), "C0")
title("Time")
