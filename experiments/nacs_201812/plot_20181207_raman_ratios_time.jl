...
const inames = ["data_20181207_085752.mat",
                "data_20181207_124232.mat",
                "data_20181207_193704.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int),
                 typemax(Int)]
const specs = [[0.1, 0.2, 0.4, 1, 1.5, 2, 3.5, 5, 8],
               OrderedDict(:tr200=>[0.1, 0.8, 1.6, 4, 6, 8, 14, 20, 32],
                           :r50=>304.24 .+ [-300; -250; -200; -150; -120;
                                            -100:10:100;
                                            120; 150; 200; 250; 300] .* 1e-3,
                           :r200=>304.24 .+ [-300; -250; -200; -150; -120;
                                               -100:10:100;
                                               120; 150; 200; 250; 300] .* 1e-3),
               OrderedDict(:tr200=>[0.1, 0.8, 1.6, 4, 6, 8, 14, 20, 32],
                           :r50=>304.24 .+ [-300; -250; -200; -150; -120;
...
data_tr50 = datas_nacs[1]
data_tr200 = [datas_nacs[2][:tr200]; datas_nacs[3][:tr200]]
data_r50 = [datas_nacs[2][:r50]; datas_nacs[3][:r50]]
data_r200 = [datas_nacs[2][:r200]; datas_nacs[3][:r200]]
...
fit_r50 = fit_survival(gen_model(0.25, 0.75), data_r50, [0.7, 304.24, 0.1])
@show fit_r50.uncs
fit_r200 = fit_survival(gen_model(0.25, 0.75), data_r200, [0.7, 304.24, 0.1])
@show fit_r200.uncs

fit_r50′ = fit_survival(model′, data_r50, [0.2, 304.24, 0.1, 0.7])
@show fit_r50′.uncs
fit_r200′ = fit_survival(model′, data_r200, [0.2, 304.24, 0.1, 0.7])
@show fit_r200′.uncs
...
grid()
ylim([0, 0.8])
text(8, 0.12, "\$\\tau=$(fit_tr50.uncs[2])\$ms", color="C0")
text(10, 0.32, "\$\\tau=$(fit_tr200.uncs[2])\$ms", color="C1")
