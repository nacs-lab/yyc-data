...
const inames = ["data_20190114_165841.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs_na = [([0, 0.5, 1, 2, 5, 10, 20, 40],
...
const prefix = joinpath(@__DIR__, "imgs", "data_20190114_165841_depump")

figure()
NaCsPlot.plot_survival_data(data_na_22, fmt="C0.", label="2, 2")
plot(fit_na_22.plotx, fit_na_22.ploty, "C0")
NaCsPlot.plot_survival_data(NaCsData.map_params((i, v)->v * 500, data_na_21),
                            fmt="C1.", label="2, 1\${}_{t\\times500}\$")
plot(fit_na_21.plotx .* 500, fit_na_21.ploty, "C1")
text(5.5, 0.26, "\$\\tau_{2, 2}=$(fit_na_22.uncs[2])ms\$", color="C0")
text(5, 0.15, "\$\\tau_{2, 1}=$(fit_na_21.uncs[2] * 1000)\\mu s\$", color="C1")
text(5, 0.03, "\$r=$(fit_na_22.uncs[2] / fit_na_21.uncs[2])\$")
