...
const inames = ["data_20190101_153121.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs_na = [0:2.5:50]
const specs_cs = [0:2.5:50]
...
fit_cs = fit_survival(model_sin0, data_cs, [0.45, 0.45, 0.026])

const prefix = joinpath(@__DIR__, "imgs", "data_20190101_153121_coprop_time")

figure()
NaCsPlot.plot_survival_data(data_na, fmt="C0.")
plot(fit_na.plotx, fit_na.ploty)
text(17, 0.82, "\$t_\\pi=$(1 / 2 / fit_na.uncs[3])\\mu s\$", color="C0")
grid()
ylim([0, 1])
xlim([0, 55])
title("Na coprop")
...
text(27, 0.85, "\$t_\\pi=$(1 / 2 / fit_cs.uncs[3])\\mu s\$", color="C0")
grid()
ylim([0, 1])
xlim([0, 55])
title("Cs coprop")
