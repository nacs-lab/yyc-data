...
const by = -0.2 .+ linspace(-2, 2, 17)
const bz = -0.005 .+ linspace(-2, 2, 17)

const inames = ["data_20190111_233624.mat"]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20190111_233624_depump_b")

function get_survival_image(xs, ys, data; offset=0)
...
function show_survival_image(xs, ys, data; offset=0, kws...)
    X, Y, ratios = get_survival_image(xs, ys, data; offset=offset)
    pcolormesh(X, Y, ratios'; shading="flat", kws...)
end

figure()
show_survival_image(by, bz, data_na)
title("Na depumping 0.05ms")
xlabel("\$B_y\$ (G)")
ylabel("\$B_z\$ (G)")
colorbar()
...
figure()
X, Y, ratios_na = get_survival_image(by, bz, data_na)
rates_na = .-log1p.(.-ratios_na ./ 0.9) ./ 0.05
pcolormesh(X, Y, rates_na'; shading="flat")
title("Na depumping rate (kHz)")
xlabel("\$B_y\$ (G)")
ylabel("\$B_z\$ (G)")
colorbar()
NaCsPlot.maybe_save("$(prefix)_na_rates")
