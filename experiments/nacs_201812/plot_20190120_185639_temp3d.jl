...
const inames = ["data_20190120_185639.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs_na = [OrderedDict(
    :az=>(-116 .+ (-30:4.0:30), 82 .+ (-30:4.0:30)),
    :rx=>(-480 .+ (-45:6.0:45), 591 .+ (-90:12.0:90)),
    :ry=>(-492 .+ (-45:6.0:45), 601 .+ (-90:12.0:90))
)]
const specs_cs = [OrderedDict(
    :az=>(-7 .+ (-15:2.0:15), 36 .+ (-15:2.0:15)),
    :rx=>(-130 .+ (-45:6.0:45), 89 .+ (-75:10.0:75)),
    :ry=>(-124 .+ (-45:6.0:45), 112 .+ (-60:8.0:60))
)]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20190120_185639_temp")
