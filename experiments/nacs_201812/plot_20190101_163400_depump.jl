...
const inames = ["data_20190101_163400.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs_cs = [([0, 0.5, 1, 2, 5, 10, 20, 40],
                   [0, 0.02, 0.05, 0.1, 0.2, 0.5])]
const specs_na = [([0, 0.5, 1, 2, 5, 10, 20, 40] ./ 2,
                   [0, 0.02, 0.05, 0.1, 0.2, 0.5] ./ 2)]
...
fit_cs_44 = fit_survival(model_expm1, data_cs_44, [0.9, 5])
fit_cs_43 = fit_survival(model_expm1, data_cs_43, [0.5, 0.06])
fit_na_22 = fit_survival(model_expm1, data_na_22, [0.9, 5])
fit_na_21 = fit_survival(model_expm1, data_na_21, [0.5, 0.06])

const prefix = joinpath(@__DIR__, "imgs", "data_20190101_163400_depump")
...
text(9, 0.42, "\$\\tau_{4, 4}=$(fit_cs_44.uncs[2])ms\$", color="C0")
text(9, 0.31, "\$\\tau_{4, 3}=$(fit_cs_43.uncs[2] * 1000)\\mu s\$", color="C1")
text(9, 0.18, "\$r=$(fit_cs_44.uncs[2] / fit_cs_43.uncs[2])\$")
...
text(6, 0.42, "\$\\tau_{2, 2}=$(fit_na_22.uncs[2])ms\$", color="C0")
text(6, 0.31, "\$\\tau_{2, 1}=$(fit_na_21.uncs[2] * 1000)\\mu s\$", color="C1")
text(6, 0.18, "\$r=$(fit_na_22.uncs[2] / fit_na_21.uncs[2])\$")
legend(fontsize="small")
grid()
ylim([0, 1])
xlim([0, 27])
