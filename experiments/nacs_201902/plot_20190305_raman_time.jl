...
const inames = ["data_20190305_141811.mat",
                "data_20190305_183528.mat",
                ]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int),
                 ]
const specs = [[0.2, 0.5, 1, 1.5, 2, 4, 7, 10, 15, 25, 40] .* 10,
               [0.2, 0.5, 1, 1.5, 2, 4, 7, 10, 15, 25, 40] .* 10,
               ]
...
fit_nacs = fit_survival(model_exp2, data_nacs, [0.45, 9, 0.3, 8000])

const prefix = joinpath(@__DIR__, "imgs", "data_20190305_raman_time")

figure()
NaCsPlot.plot_survival_data(data_nacs, fmt="C0.")
plot(fit_nacs.plotx, fit_nacs.ploty, "C0")
text(4, 0.77, "\$\\tau_{fast}=$(fit_nacs.uncs[2])\$ ms\n\$p_{fast}=$(fit_nacs.uncs[1])\$")
text(8, 0.1, "\$\\tau_{slow}=$(fit_nacs.uncs[4])\$ ms\n\$p_{slow}=$(fit_nacs.uncs[3])\$")
grid()
ax = gca()
