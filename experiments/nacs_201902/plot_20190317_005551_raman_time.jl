...
const inames = ["data_20190317_005551.mat",
                ]
...
model_exp3(x, p) = (p[1] .* exp.(x ./ -p[2]) .+ p[3] .* exp.(x ./ -p[4])
                    .+ p[5] .* exp.(x ./ -p[6]))

data_nacs = datas_nacs[1]

fit_nacs = fit_survival(model_exp3, data_nacs, [0.3, 0.4, 0.2, 4, 0.2, 100])

const prefix = joinpath(@__DIR__, "imgs", "data_20190317_005551_raman_time")

figure()
NaCsPlot.plot_survival_data(data_nacs, fmt="C0.")
plot(fit_nacs.plotx, fit_nacs.ploty, "C0")
text(0.06, 0.02, ("\$\\tau_1=$(fit_nacs.uncs[2])\$ ms\n" *
                  "\$\\tau_2=$(fit_nacs.uncs[4])\$ ms\n" *
                  "\$\\tau_3=$(fit_nacs.uncs[6])\$ ms"))
text(3, 0.5, ("\$p_1=$(fit_nacs.uncs[1])\$\n" *
              "\$p_2=$(fit_nacs.uncs[3])\$\n" *
              "\$p_3=$(fit_nacs.uncs[5])\$"))
grid()
ax = gca()
