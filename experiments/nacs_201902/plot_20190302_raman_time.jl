...
const inames = ["data_20190302_175559.mat",
                "data_20190303_004542.mat",
                ]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int),
                 ]
const specs = [[0.1, 0.5, 1, 1.5, 2, 4, 7, 10, 15, 25, 40],
               [1, 1.5, 2, 4, 7, 10, 15, 25, 40] .* 0.1,
               ]
...
fit_nacs = fit_survival(model_exp2, data_nacs, [0.45, 0.9, 0.3, 800])

const prefix = joinpath(@__DIR__, "imgs", "data_20190302_raman_time")

figure()
NaCsPlot.plot_survival_data(data_nacs, fmt="C0.")
plot(fit_nacs.plotx, fit_nacs.ploty, "C0")
text(0.45, 0.7, "\$\\tau_{fast}=$(fit_nacs.uncs[2])\$ ms\n\$p_{fast}=$(fit_nacs.uncs[1])\$")
text(1.7, 0.06, "\$\\tau_{slow}=$(fit_nacs.uncs[4])\$ ms\n\$p_{slow}=$(fit_nacs.uncs[3])\$")
grid()
ax = gca()
