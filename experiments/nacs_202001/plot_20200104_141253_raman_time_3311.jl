...
const inames = ["data_20200104_141253.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs = [[0, 0.1, 0.2, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.5, 3.5, 4.5, 5]]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20200104_141253_raman_time_3311")

function model_expoff(x, p)
    p[1] .+ p[2] .* exp.(.- x ./ p[3])
end
fit1 = fit_survival(model_expoff, datas_nacs[1], [0.39, 0.39, 1])
...
xlim([0, 5.5])
text(2.5, 0.55, "\$\\tau=$(fit1.uncs[3])\$ ms", color="C0")
# legend(fontsize="small", loc="lower right")
title("306480.7 GHz, 8 mw, 368.4353 MHz")
grid()
xlabel("Time (ms)")
