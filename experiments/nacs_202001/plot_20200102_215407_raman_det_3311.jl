...
const inames = ["data_20200102_215407.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs = [368.439 .+ (-120:15:120) .* 1e-3]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20200102_215407_raman_det_3311")
...
fit1 = fit_survival(model_lorentzian, datas_nacs[1], [0.7, 0.3, 368.439, 0.05])

figure()
NaCsPlot.plot_survival_data(datas_nacs[1], fmt="C0.", label="5 ms")
plot(fit1.plotx, fit1.ploty, "C0-")
text(368.3, 0.4, "\$f=$(fit1.uncs[3])\$ MHz", color="C0", fontsize="small")
text(368.3, 0.425, "\$\\Gamma=$(fit1.uncs[4] * 1000)\$ kHz", color="C0", fontsize="small")
# legend(fontsize="small", loc="lower right")
grid()
title("306480.7 GHz, 8 mW, 5 ms")
