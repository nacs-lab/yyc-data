...
const inames = ["data_20200131_130243.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs = [([0, 2, 5, 10, 25, 50, 100] .* 0.5,
                [0, 10, 25, 50, 100, 150] .* 10.0)]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20200131_130243_raman_time_3311")
...
fit1 = fit_survival(model_exp, datas_nacs[1][1], [0.6, 50])
fit2 = fit_survival(model_exp, datas_nacs[1][2], [0.6, 500])
...
NaCsPlot.plot_survival_data(datas_nacs[1][2], fmt="C1.", label="1 mW")
plot(fit2.plotx, fit2.ploty, "C1-")
xlim([0, 1600])
text(80, 0.22, "\$\\tau_{16}=$(fit1.uncs[2])\$ ms", color="C0")
text(700, 0.36, "\$\\tau_{1}=$(fit2.uncs[2] / 1000)\$ s", color="C1")
