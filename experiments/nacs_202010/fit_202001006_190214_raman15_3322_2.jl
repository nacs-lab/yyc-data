...
const inames = ["data_20201006_190214.mat",
                "data_20201007_192739.mat",
...
const specs = [([0], # t=0
               ...
               ([0.0], # 15 mW, 0 ms
                571.0 .+ (-8:1.6:8), # 15 mW, 0.13 ms
                571.0 .+ (-11:2.2:11), # 15 mW, 0.26 ms
                [0; [0.02, 0.04, 0.07, 0.085, 0.10, 0.13, 0.145, 0.16,
                     0.175, 0.19, 0.22, 0.25, 0.28, 0.31, 0.34, 0.37,
                     0.40, 0.43, 0.46, 0.49] .* 1.6 .- 0.01], # 15 mW, 770.571 MHz
                ),
               ([0.0, 2, 4, 10],
                [0.0, 20, 40, 80, 120],
                [0.0, 20, 40, 80, 160] .* 2,
...
const data_nacs_t = datas_nacs[2][4] # Survival 1
const data_nacs_00 = datas_nacs[2][1] # Survival 1
const data_nacs_12 = datas_nacs[2][2] # Survival 1
const data_nacs_25 = datas_nacs[2][3] # Survival 1

const data_pa_na = [datas_na[3][4]; datas_na[3][1]]
const data_pa_cs = datas_cs[3][4]
const data_pa_a = datas_nacs[3][4]
const data_pa_m = datas_nacs[3][1]

const data_nacs_m = datas_nacs[1][3]

const data_fit = [NaCsData.map_params((i, v) -> (1, 571.0, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 0.12, 1), data_nacs_12);
                  NaCsData.map_params((i, v) -> (1, v, 0.25, 1), data_nacs_25);
                  NaCsData.map_params((i, v) -> (1, 571.0, v, 1), data_nacs_t);
                  ...
                  NaCsData.map_params((i, v) -> (6, v, 0.0, 6), data_nacs_m)]
...
fit = fit_survival(model, data_fit, [571.0, 2π * 1.5, 0, 2π / 0.2, 0, 0, 0.1, 0.2,
                                     0.3, 0.8, 0.3, 0.3, 0.3, 0.15],
                   plotx=false, lower=[-Inf; zeros(13)])
@show fit.uncs
const param_1 = get_model_param(fit.param, 1)
const uncs_1 = get_model_param(fit.uncs, 1)

const plot_freq_lo = 571.0 - 12
const plot_freq_hi = 571.0 + 12
...
plot(plot_freq, model_2d.(0.25, plot_freq, (param_1,)), "C1")
NaCsPlot.plot_survival_data(data_nacs_12, fmt="C2.", label="0.12 ms")
plot(plot_freq, model_2d.(0.12, plot_freq, (param_1,)), "C2")
...
NaCsPlot.plot_survival_data(data_nacs_t, fmt="C0.", label="770.571 MHz")
plot(plot_time, model_2d.(plot_time, 571.0, (param_1,)), "C0")
title("Time")
text(0.13, 0.2, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
...
plot(plot_pa_time, model_exp(plot_pa_time, gen_pa_param(fit.param, 1, 2)), "C0-")
NaCsPlot.plot_survival_data(data_pa_cs, fmt="C1s", label="Cs")
plot(plot_pa_time, model_exp(plot_pa_time, gen_pa_param(fit.param, 2, 3)), "C1-")
NaCsPlot.plot_survival_data(data_pa_a, fmt="C2s", label="Hot")
plot(plot_pa_time, model_exp(plot_pa_time, gen_pa_param(fit.param, 3, 4)), "C2-")
NaCsPlot.plot_survival_data(data_pa_m, fmt="C3s", label="Cold")
plot(plot_pa_time, model_exp(plot_pa_time, gen_pa_param(fit.param, 4, 5)), "C3-")
xlim([0, 22])
ylim([0, 0.9])
...
plot(plot_m_time, model_ramsey(plot_m_time, gen_ramsey_param(fit.param, 6)), "C0-")
xlim([0, 0.9])
