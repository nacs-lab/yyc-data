...
const inames = ["data_20200830_004406.mat",
...
const specs = [([0.0], # 15 mW, 0 ms
               ...
               ([0.0], # 6 mW, 0 ms
                ...
                [0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16, 0.175, 0.19, 0.22, 0.25, 0.265,
                 0.28] .* 5.5 .- 0.01, # 6 mW, 770.3755 MHz
                [0.0, 2, 4, 10],
                [0.0, 20, 40, 80, 120],
                [0.0, 20, 40, 80, 160] .* 2,
                [0.0, 25, 50, 100],
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200830_004406_raman15_3322_2")
...
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.609 MHz")
plot(plot_time, model_2d.(plot_time, 609.0, (param_1,)), "C0")
title("Time")
text(0.10, 0.18, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
