...
const inames = ["data_20200811_093134.mat",
                "data_20200811_172353.mat",
...
const specs = [([0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16, 0.175, 0.19, 0.22, 0.25, 0.265,
                 0.28] * 20 .- 0.01, # 3 mW, 770.2675 MHz
                [0.0], # 3 mW, 0 ms
                267.5 .+ (-1.25:0.25:1.25), # 3 mW, 1.97 ms
                267.5 .+ (-1.5:0.3:1.5), # 3 mW, 3.94 ms
                ),
...
const data_nacs_196 = [datas_nacs[1][3]; datas_nacs[2][3]] # Survival 1
const data_nacs_393 = [datas_nacs[1][4]; datas_nacs[2][4]] # Survival 1

const data_pa_na = [datas_na[3][6]; datas_na[3][3]; datas_na[4][3]]
const data_pa_cs = datas_cs[4][3]
const data_pa_cs2 = datas_cs[3][6]
const data_pa_a = datas_nacs[4][3]
const data_pa_a2 = datas_nacs[3][6]
const data_pa_m = datas_nacs[3][3]

const data_fit = [NaCsData.map_params((i, v) -> (1, 267.5, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 1.96, 1), data_nacs_196);
                  NaCsData.map_params((i, v) -> (1, v, 3.93, 1), data_nacs_393);
                  NaCsData.map_params((i, v) -> (1, 267.5, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200811_093134_raman3_3322_2")
...
fit = fit_survival(model, data_fit, [267.5, 2π * 0.3, 0, 2π * 0.3, 0, 0, 0.2,
                                     0.30, 0.8, 0.3, 0.3, 0.3, 0.3, 0.3],
                   plotx=false, lower=zeros(14))
@show fit.uncs
const param_1 = get_model_param(fit.param, 1)
const uncs_1 = get_model_param(fit.uncs, 1)

const plot_freq_lo = 267.5 - 1.6
const plot_freq_hi = 267.5 + 1.6
...
errorbar([param_1[3]], [ratio_00], [uncs_00], fmt="C0.", label="0.0 ms")
plot([plot_freq_lo, plot_freq_hi], [v00, v00], "C0-")
NaCsPlot.plot_survival_data(data_nacs_196, fmt="C1.", label="1.96 ms")
plot(plot_freq, model_2d.(1.96, plot_freq, (param_1,)), "C1")
NaCsPlot.plot_survival_data(data_nacs_393, fmt="C2.", label="3.93 ms")
plot(plot_freq, model_2d.(3.93, plot_freq, (param_1,)), "C2")
...
subplot(2, 2, 2)
const plot_time = linspace(0, 5.70, 1000)
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.2675 MHz")
plot(plot_time, model_2d.(plot_time, 267.5, (param_1,)), "C0")
title("Time")
text(1.50, 0.20, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
