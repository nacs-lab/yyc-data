...
const inames = ["data_20200831_192240.mat",
                "data_20200830_183835.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int)]
const specs = [([0.0], # 3 mW, 0 ms
                291.2 .+ (-1.25:0.25:1.25), # 3 mW, 1.11 ms
                291.2 .+ (-1.5:0.3:1.5), # 3 mW, 2.21 ms
                [0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16, 0.175, 0.19, 0.22, 0.25, 0.265,
                 0.28] * 12 .- 0.01, # 3 mW, 770.2912 MHz
                ),
               ([0.0], # 6 mW, 0 ms
...
const data_nacs_110 = datas_nacs[1][2] # Survival 1
const data_nacs_220 = datas_nacs[1][3] # Survival 1

const data_pa_na = [datas_na[2][6 + 4]; datas_na[2][3 + 4]]
const data_pa_cs = datas_cs[2][6 + 4]
const data_pa_a = datas_nacs[2][6 + 4]
const data_pa_m = datas_nacs[2][3 + 4]

const data_fit = [NaCsData.map_params((i, v) -> (1, 291.2, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 1.10, 1), data_nacs_110);
                  NaCsData.map_params((i, v) -> (1, v, 2.20, 1), data_nacs_220);
                  NaCsData.map_params((i, v) -> (1, 291.2, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200831_192240_raman3_3322_2")
...
fit = fit_survival(model, data_fit, [291.2, 2π * 0.3, 0, 2π * 0.3, 0, 0, 0.2,
...
const plot_freq_lo = 291.2 - 1.6
const plot_freq_hi = 291.2 + 1.6
...
NaCsPlot.plot_survival_data(data_nacs_110, fmt="C1.", label="1.1 ms")
plot(plot_freq, model_2d.(1.10, plot_freq, (param_1,)), "C1")
NaCsPlot.plot_survival_data(data_nacs_220, fmt="C2.", label="2.2 ms")
plot(plot_freq, model_2d.(2.20, plot_freq, (param_1,)), "C2")
...
subplot(2, 2, 2)
const plot_time = linspace(0, 3.43, 1000)
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.2912 MHz")
plot(plot_time, model_2d.(plot_time, 291.2, (param_1,)), "C0")
title("Time")
text(0.70, 0.22, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
