...
const inames = ["data_20200810_123203.mat",
...
const specs = [([0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16, 0.175, 0.19, 0.22, 0.25, 0.265,
                 0.28] .* 8 .- 0.01, # 6 mW, 770.327914 MHz
                [0.0], # 6 mW, 0 ms
                327.9 .+ (-1.5:0.3:1.5), # 6 mW, 0.75 ms
                327.9 .+ (-2.0:0.4:2.0), # 6 mW, 1.50 ms
                ),
               ([0.0, 2, 4, 10],
                [0.0, 20, 40, 80, 120],
                [0.0, 20, 40, 80, 160] .* 2,
...
const data_nacs_74 = datas_nacs[1][3] # Survival 1
const data_nacs_149 = datas_nacs[1][4] # Survival 1

const data_pa_na = [datas_na[2][5]; datas_na[2][2]; datas_na[3][2]]
const data_pa_cs = datas_cs[3][2]
const data_pa_cs2 = datas_cs[2][5]
const data_pa_a = datas_nacs[3][2]
const data_pa_a2 = datas_nacs[2][5]
const data_pa_m = datas_nacs[2][2]

const data_fit = [NaCsData.map_params((i, v) -> (1, 327.914, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 0.74, 1), data_nacs_74);
                  NaCsData.map_params((i, v) -> (1, v, 1.49, 1), data_nacs_149);
                  NaCsData.map_params((i, v) -> (1, 327.914, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200810_123203_raman6_3322_2")
...
fit = fit_survival(model, data_fit, [327.914, 2π * 1.5, 0, 2π / 0.2, 0, 0, 0.2,
...
const plot_freq_lo = 327.914 - 2.2
const plot_freq_hi = 327.914 + 2.2
...
NaCsPlot.plot_survival_data(data_nacs_74, fmt="C1.", label="0.74 ms")
plot(plot_freq, model_2d.(0.74, plot_freq, (param_1,)), "C1")
NaCsPlot.plot_survival_data(data_nacs_149, fmt="C2.", label="1.49 ms")
plot(plot_freq, model_2d.(1.49, plot_freq, (param_1,)), "C2")
...
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.327914 MHz")
plot(plot_time, model_2d.(plot_time, 327.914, (param_1,)), "C0")
title("Time")
text(0.50, 0.20, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
...
subplot(2, 2, 4)
const img_freq = param_1[3] .+ linspace(-2, 2, 201)
const img_time = linspace(0, 2.2, 201)
