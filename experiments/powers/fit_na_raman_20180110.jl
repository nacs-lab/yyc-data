...
const iname_f2_counterop = joinpath(@__DIR__, "data", "na_f2_counter-op_20180110.csv")
const data_f2_counterop = readdlm(iname_f2_counterop, ',', Float64, skipstart=1)
const iname_f2_coprop = joinpath(@__DIR__, "data", "na_f2_coprop_20180110.csv")
const data_f2_coprop = readdlm(iname_f2_coprop, ',', Float64, skipstart=1)
const iname_f1_coprop = joinpath(@__DIR__, "data", "na_f1_coprop_20180110.csv")
const data_f1_coprop = readdlm(iname_f1_coprop, ',', Float64, skipstart=1)
const iname_f1_up = joinpath(@__DIR__, "data", "na_f1_up_20180110.csv")
const data_f1_up = readdlm(iname_f1_up, ',', Float64, skipstart=1)
const iname_f1_down = joinpath(@__DIR__, "data", "na_f1_down_20180110.csv")
const data_f1_down = readdlm(iname_f1_down, ',', Float64, skipstart=1)

const prefix = joinpath(@__DIR__, "imgs", "na_20180110")
...
fit_f2_counterop = curve_fit(model, data_f2_counterop[:, 1], data_f2_counterop[:, 2],
                             [2.5, 1.0, 3.0])
figure()
plotmax_f2_counterop = data_f2_counterop[1, 1] * 1.1
...
ylim([0, ylim()[2]])
text(0.01, 2.5, "\$a\\cdot\\sin(b\\cdot\\sin^2(c\\cdot AMP))\$", fontsize=18)
...
fit_f2_coprop = curve_fit(model, data_f2_coprop[:, 1], data_f2_coprop[:, 2],
                          [2.5, 1.0, 3.0])
figure()
plotmax_f2_coprop = data_f2_coprop[1, 1] * 1.1
...
ylim([0, ylim()[2]])
text(0.01, 1.8, "\$a\\cdot\\sin(b\\cdot\\sin^2(c\\cdot AMP))\$", fontsize=18)
...
fit_f1_coprop = curve_fit(model, data_f1_coprop[:, 1], data_f1_coprop[:, 2],
                          [2.5, 1.0, 3.0])
figure()
plotmax_f1_coprop = data_f1_coprop[1, 1] * 1.1
...
ylim([0, ylim()[2]])
text(0.01, 5, "\$a\\cdot\\sin(b\\cdot\\sin^2(c\\cdot AMP))\$", fontsize=18)
...
fit_f1_up = curve_fit(model, data_f1_up[:, 1], data_f1_up[:, 2],
                      [2.5, 1.0, 3.0])
figure()
plotmax_f1_up = data_f1_up[1, 1] * 1.1
...
ylim([0, ylim()[2]])
text(0.41, 3.7, "\$a\\cdot\\sin(b\\cdot\\sin^2(c\\cdot AMP))\$", fontsize=18)
...
fit_f1_down = curve_fit(model, data_f1_down[:, 1], data_f1_down[:, 2],
                        [2.5, 1.0, 3.0])
figure()
plotmax_f1_down = data_f1_down[1, 1] * 1.1
...
ylim([0, ylim()[2]])
text(0.01, 8, "\$a\\cdot\\sin(b\\cdot\\sin^2(c\\cdot AMP))\$", fontsize=18)
text(0.14, 0.1, params_strs(fit_f1_down), fontsize=20)
xlabel("NaRaman1/AMP")
ylabel("Power (mW)")
