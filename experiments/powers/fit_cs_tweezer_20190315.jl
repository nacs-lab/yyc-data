...
const iname_paaom = joinpath(@__DIR__, "data", "cs_tweezer_paaom_20190315.csv")
const data_paaom = readdlm(iname_paaom, ',', Float64, skipstart=1)
const iname_padpaom = joinpath(@__DIR__, "data", "cs_tweezer_padpaom_20190315.csv")
const data_padpaom = readdlm(iname_padpaom, ',', Float64, skipstart=1)
const iname_padpaom2 = joinpath(@__DIR__, "data", "cs_tweezer_padpaom2_20190315.csv")
...
const prefix = joinpath(@__DIR__, "imgs", "cs_tweezer_20190315")
...
ylim([0, ylim()[2]])
text(0.28, 15.5, "\$a_1\\cdot\\sin(b_1\\cdot\\sin^2(c_1\\cdot AMP))\$", fontsize=18, color="C0")
text(0.4, 3, params_strs(fit_paaom, "_1"), fontsize=20, color="C0")
xlabel("PAAOM/AMP")
ylabel("Power (mW)")
NaCsPlot.maybe_save("$(prefix)_paaom")

plotmax_padpaom = maximum(data_padpaom[:, 1]) * 1.3
...
text(0.365, 22 - 5, "\$a_3\\cdot\\cos^4(b_3\\cdot AMP)\$", fontsize=18, color="C1")
text(0.37, 12 - 5, params_strs2(fit_padpaom2, "_3"), fontsize=20, color="C1")
xlabel("PADPAOM/AMP")
ylabel("Power (mW)")
