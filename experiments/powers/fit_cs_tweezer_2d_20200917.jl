...
const iname_pa2d = joinpath(@__DIR__, "data", "cs_tweezer_pa2d_20200917.csv")
...
const prefix = joinpath(@__DIR__, "imgs", "cs_tweezer_20200917")
...
pcolormesh(reshape(data_pa2d[:, 1], length(pas), length(dps)),
           reshape(data_pa2d[:, 2], length(pas), length(dps)), powers_2d; shading="nearest")
xlabel("PAAOM/AMP")
ylabel("PADPaom/AMP")
colorbar()
...
gca()[:set_yscale]("log", nonpositive="clip")
grid()
