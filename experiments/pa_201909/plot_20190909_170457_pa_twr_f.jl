...
const names_files = ["names_20190909_170457.mat"]
...
for ds in [datas_cs, datas_na, datas, datas_cscs, datas_nana]
    ds[1] = NaCsData.map_params((i, v)->(v - 612) * 1000, ds[1])
end
...
const prefix = joinpath(@__DIR__, "imgs", "data_20190909_170457_pa")

fit = fit_survival(model_lorentzian, datas[1], [0.6, 0.3, 30, 100])
...
text(-210, 0.28, "\$\\Gamma=$(fit.uncs[4])\$ MHz", color="C0", fontsize="small")
text(-210, 0.37, "\$f=$(fit.uncs[3])\$ MHz", color="C0", fontsize="small")
grid()
ylim([0.23, 0.9])
legend(fontsize="small")
title("PA spectrum (two-body PA)")
xlabel("306612XXX MHz")
