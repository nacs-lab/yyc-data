...
const names_files = ["names_20190902_112810.mat",
                     "names_20190902_153514.mat"]
...
for ds in [datas_cs, datas_na, datas, datas_cscs, datas_nana]
    ds[1] = NaCsData.map_params((i, v)->(v - 496) * 1000, [ds[1]; ds[2]])
    resize!(ds, 1)
end
...
const prefix = joinpath(@__DIR__, "imgs", "data_20190902_112810_pa")

fit = fit_survival(model_lorentzian, datas[1], [0.7, 0.35, -160, 20])
...
text(-150, 0.31, "\$\\Gamma=$(fit.uncs[4])\$ MHz", color="C0", fontsize="small")
text(-150, 0.26, "\$f=$(fit.uncs[3])\$ MHz", color="C0", fontsize="small")
grid()
ylim([0.25, 0.9])
# xlim([495.95, 496.29])
