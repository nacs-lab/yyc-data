...
const inames = ["data_20200909_001528.mat",
                "data_20200910_033743.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int)]
const specs = [([0.0], # 6 mW, 0 ms
                361.9 .+ (-2.5:0.5:2.5), # 6 mW, 0.51 ms
                361.9 .+ (-3:0.6:3), # 6 mW, 1.01 ms
                [0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16, 0.175, 0.19, 0.22, 0.25, 0.265,
                 0.28] .* 5.5 .- 0.01, # 6 mW, 770.3619 MHz
                ),
               ([0.0, 2, 4, 10],
                [0.0, 20, 40, 80, 120],
                [0.0, 20, 40, 80, 160] .* 2,
...
const data_fit = [NaCsData.map_params((i, v) -> (1, 361.9, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 0.50, 1), data_nacs_50);
                  NaCsData.map_params((i, v) -> (1, v, 1.0, 1), data_nacs_100);
                  NaCsData.map_params((i, v) -> (1, 361.9, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200909_001528_raman6_3322_2")
...
fit = fit_survival(model, data_fit, [361.9, 2π * 1.5, 0, 2π / 0.2, 0, 0, 0.2,
...
const plot_freq_lo = 361.9 - 3.2
const plot_freq_hi = 361.9 + 3.2
...
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.3619 MHz")
plot(plot_time, model_2d.(plot_time, 361.9, (param_1,)), "C0")
title("Time")
