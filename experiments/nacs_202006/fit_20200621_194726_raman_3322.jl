...
const inames = ["data_20200621_194726.mat"]
...
const data_fit = [NaCsData.map_params((i, v) -> (898.0, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (v, 0.125, 1), data_nacs_125);
                  NaCsData.map_params((i, v) -> (v, 0.07, 1), data_nacs_07);
                  NaCsData.map_params((i, v) -> (898.0, v, 1), data_nacs_t)]

const prefix = joinpath(@__DIR__, "imgs", "fit_20200621_194726_raman_3322")
...
fit = fit_survival(model, data_fit, [898.0, 2π * 1.5, 0, 2π / 0.2, 0.1, 0.3],
...
title("Time")
text(0.04, 0.19, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
