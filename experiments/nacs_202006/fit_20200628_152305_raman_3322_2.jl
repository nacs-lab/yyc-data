...
const inames = ["data_20200628_152305.mat",
                "data_20200629_004904.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int)]
const specs = [([0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16, 0.175, 0.19, 0.22, 0.25, 0.265,
                 0.28] * 30 .- 0.01, # 6 mW, 770.136 MHz
                [0.0], # 6 mW, 0 ms
                136.0 .+ (-60:12:60), # 6 mW, 5 ms
                136.0 .+ (-70:14:70), # 6 mW, 10 ms
                136.0 .+ (-80:16:80), # 6 mW, 15 ms
                ),
               ([0.0, 1, 2, 4, 10],
                [0.0, 10, 20, 40, 80, 120],
                [0.0, 10, 20, 40, 80, 160] .* 2,
                [0.0, 25, 50, 100],
                [0.0, 50, 100, 200],
                [0.0, 50, 100, 200] .* 2),
               ]
...
const data_nacs_5 = datas_nacs[1][3] # Survival 1
const data_nacs_10 = datas_nacs[1][4] # Survival 1
const data_nacs_15 = datas_nacs[1][5] # Survival 1
...
const data_fit = [NaCsData.map_params((i, v) -> (1, 136.0, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 4.99, 1), data_nacs_5);
                  NaCsData.map_params((i, v) -> (1, v, 9.99, 1), data_nacs_10);
                  NaCsData.map_params((i, v) -> (1, v, 14.99, 1), data_nacs_15);
                  NaCsData.map_params((i, v) -> (1, 136.0, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200628_152305_raman_3322_2")
...
fit = fit_survival(model, data_fit, [136.0, 2π * 1.5, 0, 2π / 0.2, 0, 0, 0.1,
                                     0.3, 0.8, 0.3, 0.3, 0.3], plotx=false)
@show fit.uncs
const param_1 = get_model_param(fit.param, 1)
const uncs_1 = get_model_param(fit.uncs, 1)

const plot_freq_lo = 136.0 - 85
const plot_freq_hi = 136.0 + 85
...
NaCsPlot.plot_survival_data(data_nacs_5, fmt="C1.", label="4.99 ms")
plot(plot_freq, model_2d.(4.99, plot_freq, (param_1,)), "C1")
NaCsPlot.plot_survival_data(data_nacs_10, fmt="C2.", label="9.99 ms")
plot(plot_freq, model_2d.(9.99, plot_freq, (param_1,)), "C2")
NaCsPlot.plot_survival_data(data_nacs_15, fmt="C3.", label="14.99 ms")
plot(plot_freq, model_2d.(14.99, plot_freq, (param_1,)), "C3")
legend(fontsize="x-small", loc="lower left")
grid()
xlabel("2-Photon Detuning (770XXX kHz)")
ylabel("Two-body survival")
title("288808 GHz, 6 mW")

subplot(2, 2, 2)
const plot_time = linspace(0, 8.7, 1000)
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.136 MHz")
plot(plot_time, model_2d.(plot_time, 136.0, (param_1,)), "C0")
title("Time")
text(3.15, 0.15, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
