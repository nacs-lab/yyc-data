...
const inames = ["data_20200628_004256.mat",
                "data_20200629_004904.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int)]
const specs = [([0.04, 0.07, 0.10, 0.13, 0.16, 0.19, 0.22, 0.25,
                 0.28] * 200 .- 0.01, # 3 mW, 770.172 MHz
                [0.0], # 3 mW, 0 ms
                172.0 .+ (-30:6:30), # 3 mW, 15 ms
                172.0 .+ (-40:8:40), # 3 mW, 30 ms
                172.0 .+ (-50:10:50), # 3 mW, 60 ms
                ),
               ([0.0, 1, 2, 4, 10],
                [0.0, 10, 20, 40, 80, 120],
...
const data_nacs_30 = datas_nacs[1][4] # Survival 1
const data_nacs_60 = datas_nacs[1][5] # Survival 1
...
const data_fit = [NaCsData.map_params((i, v) -> (1, 172.0, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 14.99, 1), data_nacs_15);
                  NaCsData.map_params((i, v) -> (1, v, 29.99, 1), data_nacs_30);
                  NaCsData.map_params((i, v) -> (1, v, 59.99, 1), data_nacs_60);
                  NaCsData.map_params((i, v) -> (1, 172.0, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200628_004256_raman_3322_2")
...
fit = fit_survival(model, data_fit, [172.0, 2π * 0.1, 0, 2π * 5.0, 0, 0, 0.2,
                                     0.30, 0.8, 0.3, 0.3, 0.3], plotx=false)
@show fit.uncs
const param_1 = get_model_param(fit.param, 1)
const uncs_1 = get_model_param(fit.uncs, 1)

const plot_freq_lo = 172.0 - 55
const plot_freq_hi = 172.0 + 55
...
NaCsPlot.plot_survival_data(data_nacs_15, fmt="C1.", label="14.99 ms")
plot(plot_freq, model_2d.(14.99, plot_freq, (param_1,)), "C1")
NaCsPlot.plot_survival_data(data_nacs_30, fmt="C2.", label="29.99 ms")
plot(plot_freq, model_2d.(29.99, plot_freq, (param_1,)), "C2")
NaCsPlot.plot_survival_data(data_nacs_60, fmt="C3.", label="59.99 ms")
plot(plot_freq, model_2d.(59.99, plot_freq, (param_1,)), "C3")
legend(fontsize="x-small", loc="lower right")
grid()
xlabel("2-Photon Detuning (770XXX kHz)")
ylabel("Two-body survival")
title("288808 GHz, 3 mW")

subplot(2, 2, 2)
const plot_time = linspace(0, 58, 1000)
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.172 MHz")
plot(plot_time, model_2d.(plot_time, 172.0, (param_1,)), "C0")
title("Time")
text(17.0, 0.12, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
