...
const inames = ["data_20200626_213335.mat",
                "data_20200626_093442.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int)]
const specs = [([0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16, 0.175, 0.19, 0.22,
                 0.25, 0.265, 0.28] * 10 .- 0.01, # 6 mW, 770.092 MHz
                [0.0], # 6 mW, 0 ms
                92.0 .+ (-20:4:20), # 6 mW, 1 ms
                92.0 .+ (-25:5:25), # 6 mW, 2 ms
                92.0 .+ (-30:6:30), # 6 mW, 4 ms
                ),
               ([0.0, 2, 4, 10],
                [0.0, 40, 80, 120],
                [0.0, 40, 80, 160] .* 2,
...
const data_nacs_99 = datas_nacs[1][3] # Survival 1
const data_nacs_199 = datas_nacs[1][4] # Survival 1
const data_nacs_399 = datas_nacs[1][5] # Survival 1
...
const data_fit = [NaCsData.map_params((i, v) -> (1, 92.0, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 0.99, 1), data_nacs_99);
                  NaCsData.map_params((i, v) -> (1, v, 1.99, 1), data_nacs_199);
                  NaCsData.map_params((i, v) -> (1, v, 3.99, 1), data_nacs_399);
                  NaCsData.map_params((i, v) -> (1, 92.0, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200626_213335_raman_3322_2")
...
fit = fit_survival(model, data_fit, [92.0, 2π * 1.5, 0, 2π / 0.2, 0, 0, 0.1,
                                     0.3, 0.8, 0.3, 0.3, 0.3], plotx=false)
@show fit.uncs
const param_1 = get_model_param(fit.param, 1)
const uncs_1 = get_model_param(fit.uncs, 1)

const plot_freq_lo = 92.0 - 32
const plot_freq_hi = 92.0 + 32
...
NaCsPlot.plot_survival_data(data_nacs_99, fmt="C1.", label="0.99 ms")
plot(plot_freq, model_2d.(0.99, plot_freq, (param_1,)), "C1")
NaCsPlot.plot_survival_data(data_nacs_199, fmt="C2.", label="1.99 ms")
plot(plot_freq, model_2d.(1.99, plot_freq, (param_1,)), "C2")
NaCsPlot.plot_survival_data(data_nacs_399, fmt="C3.", label="3.99 ms")
plot(plot_freq, model_2d.(3.99, plot_freq, (param_1,)), "C3")
legend(fontsize="x-small", loc="lower left")
grid()
xlabel("2-Photon Detuning (770XXX kHz)")
ylabel("Two-body survival")
title("288800 GHz, 6 mW")

subplot(2, 2, 2)
const plot_time = linspace(0, 2.8, 1000)
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.092 MHz")
plot(plot_time, model_2d.(plot_time, 92.0, (param_1,)), "C0")
title("Time")
text(0.8, 0.15, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
