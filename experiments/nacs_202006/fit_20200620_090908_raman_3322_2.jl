...
const inames = ["data_20200620_090908.mat",
                "data_20200620_143850.mat",
                "data_20200621_001933.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int),
                 typemax(Int)]
const specs = [([0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16, 0.175,
                 0.19, 0.22, 0.25, 0.265, 0.28] .* 2.5 .- 0.01, # 6 mW, 770.48748 MHz
                [0.0], # 6 mW, 0 ms
                487.48 .+ (-3.5:0.7:3.5), # 6 mW, 0.23 ms
                487.48 .+ (-3.5:0.7:3.5), # 6 mW, 0.42 ms
                ),
               ([0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16, 0.175,
                 0.19, 0.22, 0.25, 0.265, 0.28] .* 2.5 .- 0.01, # 6 mW, 770.48993 MHz
                [0.0], # 6 mW, 0 ms
                489.93 .+ (-3.5:0.7:3.5), # 6 mW, 0.23 ms
                489.93 .+ (-4.5:0.9:4.5), # 6 mW, 0.42 ms
                ),
               ([0.0, 2, 4, 10],
                [0.0, 40, 80, 120],
                [0.0, 40, 80, 160] .* 2,
...
const data_nacs_t_2 = datas_nacs[2][1] # Survival 6
const data_nacs_00_2 = datas_nacs[2][2] # Survival 6
const data_nacs_22_2 = datas_nacs[2][3] # Survival 6
const data_nacs_41_2 = datas_nacs[2][4] # Survival 6

const data_pa_na = [datas_na[3][5]; datas_na[3][2]]
const data_pa_cs = datas_cs[3][5]
const data_pa_a = datas_nacs[3][5]
const data_pa_m = datas_nacs[3][2]

const data_fit = [NaCsData.map_params((i, v) -> (1, 489.93, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 0.22, 1), data_nacs_22);
                  NaCsData.map_params((i, v) -> (1, v, 0.41, 1), data_nacs_41);
                  NaCsData.map_params((i, v) -> (1, 487.48, v, 1), data_nacs_t);
                  NaCsData.map_params((i, v) -> (1, 489.93, v, 6), data_nacs_00_2);
                  NaCsData.map_params((i, v) -> (1, v, 0.22, 6), data_nacs_22_2);
                  NaCsData.map_params((i, v) -> (1, v, 0.41, 6), data_nacs_41_2);
                  NaCsData.map_params((i, v) -> (1, 489.93, v, 6), data_nacs_t_2);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200620_090908_raman_3322_2")
...
fit = fit_survival(model, data_fit, [489.93, 2π * 1.5, 0, 2π / 0.2, 0, 0, 0.1,
                                     0.3, 0.8, 0.3, 0.3, 0.3, 0.3],
                   plotx=false, lower=zeros(13))
@show fit.uncs
const param_1 = get_model_param(fit.param, 1)
const param_6 = get_model_param(fit.param, 6)
const uncs_1 = get_model_param(fit.uncs, 1)

const plot_freq_lo = 489.93 - 8
const plot_freq_hi = 489.93 + 8
...
ratio_00_2, uncs_00_2 = get_ratio_val(data_nacs_00_2)
v00_2 = model_2d(0, 0, param_6)
errorbar([param_1[3]], [ratio_00_2], [uncs_00_2], fmt="C3.", label="0.00 ms")
plot([plot_freq_lo, plot_freq_hi], [v00_2, v00_2], "C3-")
NaCsPlot.plot_survival_data(data_nacs_22_2, fmt="C4.", label="0.22 ms")
plot(plot_freq, model_2d.(0.22, plot_freq, (param_6,)), "C4")
NaCsPlot.plot_survival_data(data_nacs_41_2, fmt="C5.", label="0.41 ms")
plot(plot_freq, model_2d.(0.41, plot_freq, (param_6,)), "C5")
...
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.48748 MHz")
plot(plot_time, model_2d.(plot_time, 487.48, (param_1,)), "C0")
NaCsPlot.plot_survival_data([data_nacs_00_2; data_nacs_t_2], fmt="C1.", label="770.48993 MHz")
plot(plot_time, model_2d.(plot_time, 489.93, (param_6,)), "C1")
title("Time")
text(0.15, 0.30, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
