...
const inames = ["data_20200621_101229.mat",
                "data_20200621_001933.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int)]
const specs = [([0.04, 0.07, 0.085, 0.10, 0.115, 0.13, 0.16,
                 0.175, 0.19, 0.22, 0.25] .* 0.8 .- 0.01, # 15 mW, 770.898 MHz
                [0.0], # 15 mW, 0 ms
                898 .+ (-20:4.0:20), # 15 mW, 0.135 ms
                898 .+ (-15:3.0:15), # 15 mW, 0.08 ms
                ),
               ([0.0, 2, 4, 10],
                [0.0, 40, 80, 120],
                [0.0, 40, 80, 160] .* 2,
...
const data_fit = [NaCsData.map_params((i, v) -> (1, 898.0, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 0.125, 1), data_nacs_125);
                  NaCsData.map_params((i, v) -> (1, v, 0.07, 1), data_nacs_07);
                  NaCsData.map_params((i, v) -> (1, 898.0, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200621_101229_raman_3322_2")
...
fit = fit_survival(model, data_fit, [898.0, 2π * 1.5, 0, 2π / 0.2, 0, 0, 0.1,
                                     0.3, 0.8, 0.3, 0.3, 0.3],
...
const plot_freq_lo = 898.0 - 21
const plot_freq_hi = 898.0 + 21
...
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.898 MHz")
plot(plot_time, model_2d.(plot_time, 898.0, (param_1,)), "C0")
title("Time")
text(0.04, 0.22, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
