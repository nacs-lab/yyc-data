...
const powers = [15, 6, 3]
const omega_r = [8.08, 2.448, 1.048] # kHz
const omega_r_unc = [0.14, 0.055, 0.017]
const gamma_m = [3.01, 0.689, 0.376] # kHz
const gamma_m_unc = [0.19, 0.085, 0.025]
const gamma_a = [16.0, 2.18, 0.568] # Hz
const gamma_a_unc = [1.9, 0.18, 0.067]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20200618_scaling_3322")
...
text(5.8, 1.9, "\$n=$(fit_omega_r_p.uncs[2])\$", color="C2")
legend()
grid()
xscale("log")
yscale("log")
...
text(5.2, 0.30, "\$n=$(fit_gamma_m_p.uncs[2])\$", color="C2")
legend()
grid()
xscale("log")
yscale("log")
