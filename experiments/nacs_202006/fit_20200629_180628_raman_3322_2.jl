...
const inames = ["data_20200629_180628.mat",
                "data_20200629_004904.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int),
                 typemax(Int)]
const specs = [([0.04, 0.07, 0.10, 0.13, 0.16, 0.19, 0.22, 0.25] * 20 .- 0.01, # 15 mW, 770.030 MHz
                [0.0], # 15 mW, 0 ms
                30.0 .+ (-100:20:100), # 15 mW, 1.7 ms
                30.0 .+ (-100:20:100), # 15 mW, 3.5 ms
                30.0 .+ (-150:30:150), # 15 mW, 5.0 ms
                ),
               ([0.0, 1, 2, 4, 10],
                [0.0, 10, 20, 40, 80, 120],
...
const data_nacs_169 = datas_nacs[1][3] # Survival 1
const data_nacs_349 = datas_nacs[1][4] # Survival 1
const data_nacs_499 = datas_nacs[1][5] # Survival 1
...
const data_fit = [NaCsData.map_params((i, v) -> (1, 30.0, v, 1), data_nacs_00);
                  NaCsData.map_params((i, v) -> (1, v, 1.69, 1), data_nacs_169);
                  NaCsData.map_params((i, v) -> (1, v, 3.49, 1), data_nacs_349);
                  NaCsData.map_params((i, v) -> (1, v, 4.99, 1), data_nacs_499);
                  NaCsData.map_params((i, v) -> (1, 30.0, v, 1), data_nacs_t);
...
const prefix = joinpath(@__DIR__, "imgs", "fit_20200629_180628_raman_3322_2")
...
fit = fit_survival(model, data_fit, [30.0, 2π * 1.0, 0, 2π / 0.1, 0, 0, 0.1,
                                     0.3, 0.6, 0.3, 0.3, 0.3], plotx=false)
@show fit.uncs
const param_1 = get_model_param(fit.param, 1)
const uncs_1 = get_model_param(fit.uncs, 1)

const plot_freq_lo = 30.0 - 160
const plot_freq_hi = 30.0 + 160
...
NaCsPlot.plot_survival_data(data_nacs_169, fmt="C1.", label="1.69 ms")
plot(plot_freq, model_2d.(1.69, plot_freq, (param_1,)), "C1")
NaCsPlot.plot_survival_data(data_nacs_349, fmt="C2.", label="3.49 ms")
plot(plot_freq, model_2d.(3.49, plot_freq, (param_1,)), "C2")
NaCsPlot.plot_survival_data(data_nacs_499, fmt="C3.", label="4.99 ms")
plot(plot_freq, model_2d.(4.99, plot_freq, (param_1,)), "C3")
legend(fontsize="x-small", loc="upper left")
grid()
xlabel("2-Photon Detuning (770XXX kHz)")
ylabel("Two-body survival")
title("288808 GHz, 15 mW")

subplot(2, 2, 2)
const plot_time = linspace(0, 5.5, 1000)
NaCsPlot.plot_survival_data([data_nacs_00; data_nacs_t], fmt="C0.", label="770.030 MHz")
plot(plot_time, model_2d.(plot_time, 30.0, (param_1,)), "C0")
title("Time")
text(0.80, 0.19, ("\$f_{res}=$(uncs_1[3] / 1000 + 770)\$ MHz\n" *
