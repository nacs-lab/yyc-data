...
const inames = ["data_20190723_174531.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs_na = [OrderedDict(
    :az=>(-109 .+ (-30:4.0:30), 79 .+ (-30:4.0:30)),
    :rx=>(-521 .+ (-45:6.0:45), 519 .+ (-90:12.0:90)),
    :ry=>(-517 .+ (-45:6.0:45), 523 .+ (-90:12.0:90))
)]
const specs_cs = [OrderedDict(
    :az=>(-12 .+ (-15:2.0:15), 32 .+ (-15:2.0:15)),
    :rx=>(-136 .+ (-45:6.0:45), 120 .+ (-75:10.0:75)),
    :ry=>(-131 .+ (-45:6.0:45), 111 .+ (-60:8.0:60))
)]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20190723_174531_temp")
