...
const inames = ["data_20200229_033442.mat",
                "data_20200229_092231.mat"]
...
const specs_cs = [([0, 0.5, 1, 2, 5, 10, 20] .* 50,
                   [0, 0.01, 0.02, 0.05, 0.1, 0.2, 0.4] .* 5),
                  [0.2, 0.4] .* 20]
...
data_cs_43 = [datas_cs[1][2]; datas_cs[2]]
data_na_22 = datas_na[1][1]
data_na_21 = [datas_na[1][2]; datas_na[2]]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20200229_033442_depump")
...
text(100, 0.29, "\$\\tau_{2, 2}=$(fit_na_22.uncs[2])ms\$", color="C0")
text(200, 0.58, "\$\\tau_{2, 1}=$(fit_na_21.uncs[2] * 1000)\\mu s\$", color="C1")
