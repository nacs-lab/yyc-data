...
function load_data(names, selector)
    ...
    for name in names
        param, counts = matopen(joinpath(@__DIR__, "data", "data_$name.mat")) do fd
            ...
            amps = [16.0, 8.0] # sg["base"]["vars"]["params"]["Merge"]["Cs"]["Wait"]["PTwzr"]
...
const names_files = ["names_20200215_101243.mat"]
...
for ds in [datas_cs, datas_na, datas, datas_cscs, datas_nana]
    ds[1] = NaCsData.map_params((i1, i2, v)->(v - 306000), ds[1])
end
...
const prefix = joinpath(@__DIR__, "imgs", "data_20200215_101243_pa_492")

# fit_8 = fit_survival(model_lorentzian, datas[1][:, 1], [0.7, 0.35, -20, 50])
# fit_16 = fit_survival(model_lorentzian, datas[1][:, 2], [0.7, 0.35, 850, 50])

figure()
NaCsPlot.plot_survival_data(datas[1][:, 1], fmt="C0.-", label="8 mW")
# plot(fit_8.plotx, fit_8.ploty, "C0")
NaCsPlot.plot_survival_data(datas[1][:, 2], fmt="C1.-", label="16 mW")
# plot(fit_16.plotx, fit_16.ploty, "C1")
# text(-175, 0.78, "\$\\Gamma_{8}=$(fit_8.uncs[4])\$ MHz", color="C0", fontsize="small")
# text(-175, 0.72, "\$f_{8}=$(fit_8.uncs[3])\$ MHz", color="C0", fontsize="small")
# text(750, 0.66, "\$\\Gamma_{16}=$(fit_16.uncs[4])\$ MHz", color="C1", fontsize="small")
# text(750, 0.6, "\$f_{16}=$(fit_16.uncs[3])\$ MHz", color="C1", fontsize="small")
grid()
ylim([0.1, 0.9])
# xlim([495.94, 496.16])
...
subplot(2, 2, 4)
NaCsPlot.plot_survival_data(datas_cscs[1][:, 1], fmt="C2.--", label="Cs 8 mW")
NaCsPlot.plot_survival_data(datas_cscs[1][:, 2], fmt="C2.:", label="Cs 16 mW")
NaCsPlot.plot_survival_data(datas_nana[1][:, 1], fmt="C1.--", label="Na 8 mW")
NaCsPlot.plot_survival_data(datas_nana[1][:, 2], fmt="C1.:", label="Na 16 mW")
grid()
ylim([0.1, 0.9])
# xlim([495.94, 496.16])
