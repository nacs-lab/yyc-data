...
const inames = ["data_20191221_100351.mat"]
const datas = [NaCsData.load_striped_mat(joinpath(@__DIR__, "data", iname)) for iname in inames]
const maxcnts = [typemax(Int)]
const specs_na = [OrderedDict(
    :az=>(-91 .+ (-30:4.0:30), 90 .+ (-30:4.0:30)),
    :rx=>(-484 .+ (-45:6.0:45), 516 .+ (-90:12.0:90)),
    :ry=>(-479 .+ (-45:6.0:45), 510 .+ (-90:12.0:90))
)]
const specs_cs = [OrderedDict(
    :az=>(16 .+ (-15:2.0:15), 55 .+ (-15:2.0:15)),
    :rx=>(-92 .+ (-45:6.0:45), 120 .+ (-75:10.0:75)),
    :ry=>(-92 .+ (-45:6.0:45), 111 .+ (-60:8.0:60))
)]
...
const prefix = joinpath(@__DIR__, "imgs", "data_20191221_100351_temp")
