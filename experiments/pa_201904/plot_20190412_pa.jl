...
const names_files = ["data_20190412_pa_names.mat",
                     "data_20190413_pa_names.mat",
                     "data_20190414_pa_names.mat"]
const datas = [load_data(matopen(fd->read(fd, "names"), joinpath(@__DIR__, "data", names_file)),
                         NaCsData.select_single((1, 2,), (3, 4,)))
               for names_file in names_files]

const prefix = joinpath(@__DIR__, "imgs", "data_20190412_pa")

figure()
NaCsPlot.plot_survival_data(datas[1], fmt="C0.-", label="1ms")
NaCsPlot.plot_survival_data(datas[2], fmt="C0.-")
NaCsPlot.plot_survival_data(datas[3], fmt="C1.-", label="10ms")
legend()
grid()
title("PA spectrum")
xlabel("288XXX GHz")
