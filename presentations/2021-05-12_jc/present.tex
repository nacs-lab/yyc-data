...
\title{Entanglement from tensor networks on a trapped-ion QCCD quantum computer}
\date{May 12, 2021}
\author{Yichao Yu}
\institute{Ni Group}
...
\begin{frame}{Matrix product states (MPS)}
  \begin{center}
    \begin{tikzpicture}
      \node at (0, 0) {$\sum c_{\sigma_1\sigma_2\cdots\sigma_n}|\sigma_1\sigma_2\cdots\sigma_n\rangle$};

      \visible<2->{
        \node at (0, -0.7) {$\sum \mathrm{Tr}\paren{V_{\sigma_1}V_{\sigma_2}\cdots V_{\sigma_n}}|\sigma_1\sigma_2\cdots\sigma_n\rangle$};
      }
      \visible<3->{
      ...
      \visible<4->{
        ...
        {Minimum matrix dimension:\\
          related to the entanglement in the system};
      }
      \visible<5->{
        \node[align=left] at (0, -4.5) {Exploding structure in the state};
      }
    \end{tikzpicture}
  \end{center}
\end{frame}
