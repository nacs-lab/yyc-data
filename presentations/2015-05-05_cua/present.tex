...
% IMP: Could like to find highest power
  ...
  \begin{columns}
    ...
    \begin{center}
      \begin{block}{Laser diode from Innolume}
        \begin{itemize}
        \item<2-> Tunable over $>100$nm: $1175$-$1280$nm
        \item<3-> Max power: $200$mW\\
          Max current: $500$mA
        \end{itemize}
      \end{block}
      \begin{tikzpicture}
...
% IMP: Shorten the description of setup
% IMP: Emphasise size and vapor pressure (so no Zeeman slower) (so more sensitive)

% IMP: Video: describe what it is.
% IMP: Ready for next step
...
\begin{frame}[t]{MOT stability}
  \begin{center}
    ...
    \only<4>{
      \vspace{-0.4cm}
      \movie[height = 8cm,width = 10cm,showcontrols,poster]{}{Piezo.mp4}
    }
  \end{center}
\end{frame}
