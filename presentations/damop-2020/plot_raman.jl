...
figure()
l1 = plot(data[:, 1] .- 351271.53, abs.(data[:, 2] ./ 2π / 1000), "C0", label="\$\\Omega_{Raman}\$")
l2 = plot(data[:, 1] .- 351271.53, abs.(data[:, 4] ./ 2π / 1000), "C1", label="\$\\Gamma_{scatter}\$")
ylabel("\$2\\pi\\cdot \\mathrm{kHz}\$")
xlabel("One-Photon Detuning (GHz)")
ylim([0, 800])
grid()
ax = gca()
ax2 = ax.twinx()
l3 = ax2.plot(data[:, 1] .- 351271.53, abs.(data[:, 2] ./ data[:, 4]), "C2", label="\$\\frac{\\Omega_{Raman}}{\\Gamma_{scatter}}\$")
ls = [l1; l2; l3]
