...
% Difficulty: Have to bridge the size gap
% Solution: intraction
% % Approach 1: FB resonance
% % Approach 2: Optical transfer using the natural scattering length.
% Servey of interaction shift: (see paper for detail)
% % Found 33+22 (increase in coupling)
% % Good for initial state prep (cooling/merging does not have to be optimized at all time)
% Raman transfer scheme
% % Binding energy prediction from
% % Experimentally measured to be ... .
% Can we coherently transfer?
% % Rabi flopping signal.
% Outlook: optimize, lifetime issue
...
% Fortunately, although we couldn't really push the atoms closer with external confinements,
% there are other ways to bring the atoms closer, namely, by using the interactions between
% the atoms. In our experiment, we take advantage of this interaction with two approaches,
% 1. By tuning the interaction FB resonances. This is the tranditional method for making
% % molecules in bulk gas and we have reciently shown that this can be done efficiently
% % in optical tweezers. (See arxiv or published paper, also poster)
% 2. The other approach that I'm going to focus on today, is to use the intrinsic
% % scattering length between the atoms and create the molecule using optical transfer.
% % Advantages:
% % * No big magnetic coils to generate ~1000G field, more compact
% % * Faster molecule creation.

% As preparation for the molecule creation, we measured interaction shift to characterize
% the scattering lengths of the system. (Result in recent publication XXX)
% 1. Found a state with large scattering length (show wavefunction size)
% 2. Another advantage that we only realized after doing the measurement.
% % Spectrum when driving a Raman transition. Single peak with no
% % Center peak is no interaction, left is actually a sideband that can be ignored for now.
% % Right peak is the interaction shifted peak.

% Raman transfer
% State preparation
% Scattering length vs wavefunction overlap
% Transfer to molecular state
