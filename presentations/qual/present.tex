    ...
    \begin{center}
      \begin{tikzpicture}[scale=4]
        \foreach \i in {0,0.5,0.9,1.05,1.25,1.4,1.8,2.3} {
          \draw[gray!40,dashed,line width=0.5] (\i, 0) -- (\i, 1.3);
        }
        \draw[line width=1.5,->] (-0.1, 0) -- (2.5, 0) node[below] {$t/ms$};
        \foreach \i in {0,5,...,20} {
          \draw[line width=1] (\i/10, 0) -- (\i/10,-0.015);
          \path (\i/10, 0) node[below] {$\i$};
        }
        \foreach \i in {0,...,23} {
          \draw (\i/10, 0) -- (\i/10,-0.01);
        }
        ...
        -- (2.3, 0.7);
        \draw[line width=1,orange] (1.9, 1.1) -- (2.05, 1.1) node[right] {$700$nm (Na)};
        \draw[line width=1,blue] (1.9, 0.8) -- (2.05, 0.8) node[right] {$976$nm (Cs)};

        \path (0, 0.3) node[above,rotate=90] {Trap depth};
        \path (0, 0.95) node[above,rotate=90] {Position};
