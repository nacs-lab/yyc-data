...
\pgfooclass{tweezer}{
  ...
  \method drawTweezer(#1,#2) {
    \begin{pgfonlayer}{tweezer}
      \shade[shading=radial,path fading=glow fading,shift={(#1,#2)},rotate=90,yscale=1,
      fill opacity=0.9,inner color=orange!50!yellow]
      plot[draw,samples=200,domain=-2.5:2.5] function {sqrt(0.01 + x**2 / 10)}
      -- plot[draw,samples=200,domain=2.5:-2.5] function {-sqrt(0.01 + x**2 / 10)};
    \end{pgfonlayer}
  }
  \method up(#1,#2) {
    \pgfoothis.drawTweezer(#1,#2);
    \fill [blue,path fading=glow fading] (#1,#2+0.06) circle (0.12);
    \fill [red,path fading=glow fading] (#1,#2-0.06) circle (0.16);
  }
  \method down(#1,#2) {
    \pgfoothis.drawTweezer(#1,#2);
    \fill [red,path fading=glow fading] (#1,#2+0.06) circle (0.16);
...
\begin{frame}{Molecules in optical tweezer}
  \begin{columns}
    ...
    \begin{block}{Features}
      ...
      \item<3-> High filling fraction
      \item<4-> Single site detection and manipulation
      \end{itemize}
    \end{block}
    \column{7cm}
    \begin{center}
...
% * Make molecule
%
% Achieve with optical tweezers
% * Use established atomic cooling techniques
% * Rearranging for high filling
% * Take adtantage of molecules
