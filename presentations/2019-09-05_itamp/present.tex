  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \visible<3-> {
        ...
        \begin{scope}[shift={(-4.5, -5.5)}, scale=0.8]
          \mytweezer.up(0, 0)
...
\begin{frame}{Real Issue with Na: Light Shift}
  ...
  \begin{columns}
    \column{5.7cm}
    \begin{center}
      \only<-2>{\includegraphics[width=5.5cm]{light-shift.png}}
      \only<3->{
...
\begin{frame}{Raman Sideband Cooling}
  \begin{tikzpicture}
    ...
    \visible<3> {
      \node at (-3, 1) {\includegraphics[width=5.5cm]{raman_rabi_1}};
    }
    \visible<4-> {
      \node at (-3, 1) {\includegraphics[width=5.5cm]{raman_rabi_all}};
    }
    \visible<5-> {
      \node[red] at (-3, -2.5) {3D ground state: $93.5(7)\%$};
    }
    \visible<2-4> {
      \node[align=center] at (3, 0) {
...
\begin{frame}[t]{\alt<-3>{Photoassociation (PA) Spectroscopy}{Electromagnetically Induced Transparency (EIT) Spectroscopy}}
  \vspace{-0.4cm}
  \begin{center}
    \begin{tikzpicture}
...
\begin{frame}{Optical Transfer to Weakly-Bound Molecular}
  \begin{center}
    \begin{tikzpicture}
      ...
      \visible<4-> {
        \node[align=center] at (9, 4.5) {But no Rabi oscillation\\
          and $\Gamma_e$ is about\\
          $50\sim100$x theory prediction.};
      }
      \visible<5-> {
        % TODO width comparison
        \node[align=center] at (3, 4) {
