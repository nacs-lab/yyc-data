...
\begin{frame}
  \begin{columns}
    ...
    \begin{block}{Light shift}
      \begin{itemize}
      \item<2-> Inefficient cooling;\\
        Heating
      \item<2-> Shift imaging light out of resonance
      \end{itemize}
    \end{block}
    \visible<1-> {
      \begin{center}
    ...
    \begin{center}
      \begin{tikzpicture}
        \visible<2-5> {
          ...
          \visible<3-> {
            \foreach \w in {20,18,...,1}
            {\draw[orange,line width=\w,opacity=0.1]
              plot[samples=200,domain=-2:2]
              function {2.75 - exp(-x**2 / 2 / (0.8**2))};}
          }
...
\begin{frame}
  \begin{columns}
    ...
    \begin{block}{Trap modulation}
      \begin{itemize}
      \item<2-> Alternate between resonant (cooling and imaging) and trap light
      \item<3-> Switching at $1\sim3$MHz\\
