...
figure()
errorbar(data[:, 1], data[:, 2], data[:, 3],
         label="DC,\$\\lambda=970nm\$", fmt="o-", linewidth=3, markersize=8)
errorbar(data2[:, 1], data2[:, 2], data2[:, 3],
         label="DC,\$\\lambda=935nm\$", fmt="o-", linewidth=3, markersize=8)
ylim([0, 30000])
legend()
grid()
xlabel("Detuning (MHz)", size=20)
...
errorbar(data3[:, 1], data3[:, 2], data3[:, 3],
         label="AC,\$\\lambda=970nm\$", fmt="o-", linewidth=3, markersize=8)
ylim([0, 30000])
legend()
grid()
xlabel("Detuning (MHz)", size=20)
