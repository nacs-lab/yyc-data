...
(note that the $\Delta$ is \textbf{not} detuning). Formally the time evolution is
\eqar{
  &\exp\paren{-\ui H't}\\
...
In the underdamped regime, define $\Omega'=\sqrt{\Omega^2-\Delta^2}$
\eqar{
  &\exp\paren{-\ui H't}\\
...
Verify the solution, time derivative
\eqar{
...
Total decay proobability for both states is
\eqar{
  &p_1(T)+p_2(T)\\
